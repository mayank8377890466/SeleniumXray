pipeline {
   ...
   parameters {
     ...
     string defaultValue: '0', description: 'Enter thread count', name: 'maxSessions'
   }
    stages {
    stage("SonarQube Analysis"){
	  ...
	  stage("Start Grid"){
		...
		stage("Run Test"){
			steps{
				sh "mvn clean test -Dbrowser='${params.browser}' -Dgrid_hub='${params.grid_hub}' -DmaxSessions='${params.maxSessions}'"
			}
		}
	}
	post{
